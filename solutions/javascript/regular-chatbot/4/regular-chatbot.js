...
export function removeEmoji(message) {
  ...
  * la d* se refiere a cualquier letra o numero que venga despues
  * g (global) -> todas las apariciones
  */
  
  //const regex = new RegExp('\\bemoji\\d*','g'); // Constructor Syntax

  const regex = /\bemoji\d*/g;
...
export function niceToMeetYou(fullName) {
  ...
  const regex = /(\p{Letter}+)\p{White_Space}*,\p{White_Space}*(\p{Letter}+)/u
